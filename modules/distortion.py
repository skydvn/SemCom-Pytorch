...
#         distortion_metric = getattr(args, 'distortion_metric', 'MSE')  # Lấy giá trị mặc định nếu không tồn tại
...
class MSE(nn.Module):
    ...
    def forward(self, X, Y):
        if self.normalization:
            X = (X + 1) / 2
            Y = (Y + 1) / 2  # X và Y thuộc [-1,1] nên chuẩn hóa lại về 0,1 
        return torch.mean(self.squared_difference(X * 255., Y * 255.))

class Distortion(nn.Module):
    def __init__(self, args=None):
        super(Distortion, self).__init__()
        self.dist = MSE(normalization=False)  # Mặc định sử dụng MSE
