...
from models.djsccq import *


class DJSCCQTrainer(BaseTrainer):
    def __init__(self, args):
        super().__init__(args)

        self.model = DJSCCQ_CIFAR(self.args, self.in_channel, self.class_num).to(self.device)
        self.optimizer = Adam(self.model.parameters(), lr=self.args.lr)
        self.criterion = DJSCCQLoss()
        ...
        for epoch in range(self.args.out_e):
            ...
            for x, y in tqdm(self.train_dl):
                ...
                """ 
                    Measure p_c
                    - Params: z: (16x3x3) 
                    - c_j (QAM-4): (4x1): 0001 0010
                    -> Use Eq. (18) 
                    
                """
                p_c = 0 + 0 + 0 + 0
                loss = self.criterion.forward(self.args, x, rec, p_c)
...
class DJSCCQLoss(nn.Module):
    ...
    def forward(self, args, rec, img, p_c):
        """
        :param args:
        :param rec:
        :param img:
        :param P(C): Approximated from Eq. (18)
        :param U(C): If QAM-4 -> has 4 nodes -> U_c = 1/4
        :return: Loss as in Eq. (21)
        """
